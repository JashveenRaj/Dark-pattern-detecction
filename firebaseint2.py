...
config = {
  "apiKey": "AIzaSyCNn7v1-elDtbE2DuA5qM8QMvKOEY6slQk",
  "authDomain": "dark-pattern-buster.firebaseapp.com",
  "databaseURL": "https://dark-pattern-buster-default-rtdb.firebaseio.com",
  "projectId": "dark-pattern-buster",
  "storageBucket": "dark-pattern-buster.appspot.com",
  "messagingSenderId": "136176043854",
  "appId": "1:136176043854:web:c6099fe6bce8dcf200801f",
  "measurementId": "G-LDH9SHCYYY"
};
...
products = soup.find_all(class_='ebayui-ellipsis-2')
statuses = soup.find_all(class_='dne-itemcard-hotness itemcard-hotness-red ')
...
# Upload the results to Firebase Realtime Database
for idx, (text, label) in enumerate(zip(text_lines, predicted_labels)):
    if 'Almost gone' in text:
        detected_product = {"name": text, "label": label, "potential_dark_pattern": True}
    else:
        detected_product = {"name": text, "label": label, "potential_dark_pattern": False}
    db.child("detected_products").push(detected_product)
